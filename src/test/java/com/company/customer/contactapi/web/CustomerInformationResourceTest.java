...
public class CustomerInformationResourceTest {
    ...
    public void getUniqueCustomerContactException() throws Exception {
        when(customerContactService.getCustomerById(anyInt())).thenThrow(new BeanIsAbstractException("dataSource"));
        mockMvc.perform(get("/contacts/123")
                .accept(MediaType.parseMediaType("application/json;charset=UTF-8")))
                .andExpect(status().isOk())
                .andExpect(content().contentType("application/json;charset=UTF-8"));
    ...
    public void postNewCustomerContact() throws Exception {
        when(customerContactService.postCustomer(any(CustomerContactEntity.class))).thenReturn(customerContactEntity);
        Gson gson = new Gson();
